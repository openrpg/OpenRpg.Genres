...
namespace OpenRpg.Genres.UnitTests.Persistence
{
    public class InventoryBuilderAndMapperSanityTests
    {
        ...
        public void should_build_and_map_correctly()
        {
            ...
            var dummyItem1 = new DefaultItem() { UniqueId = Guid.NewGuid(), ItemTemplate = dummyItemTemplate1 };
            dummyItem1.Variables.Amount(10);
            
            var dummyItemTemplate2 = new DefaultItemTemplate() { Id = 2 };
            var dummyItem2 = new DefaultItem() { UniqueId = Guid.NewGuid(), ItemTemplate = dummyItemTemplate2 };
            ...
            ((DefaultItem)regeneratedInventory.Items.Single(x => x.Variables.Amount() == 10)).ItemTemplate = dummyItemTemplate1;
            ((DefaultItem)regeneratedInventory.Items.Single(x => x.Variables.Amount() == 25)).ItemTemplate = dummyItemTemplate2;
